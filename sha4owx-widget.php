...
(function () {
  var SK = 'sha4owx_ref';
  var hasRef = location.search.indexOf('ref=sha4owx') !== -1;
  var hasReferrer = document.referrer && document.referrer.indexOf('sha4owx.in') !== -1;

  // Persist the referral flag in sessionStorage (with fallback)
  var stored = false;
  try {
    if (hasRef || hasReferrer) sessionStorage.setItem(SK, '1');
    stored = sessionStorage.getItem(SK) === '1';
  } catch (e) {
    stored = hasRef || hasReferrer;
  }
  ...
  function rewriteLinks() {
    ...
    for (var i = 0; i < links.length; i++) {
      try {
        var h = links[i].href;
        if (h && h.indexOf(location.hostname) !== -1 && h.indexOf('ref=sha4owx') === -1) {
          links[i].href = h + (h.indexOf('?') !== -1 ? '&' : '?') + 'ref=sha4owx';
        }
      } catch (e) {}
    }
  }
  ...
  var wa = { phone: "916358006660", message: "Hi! I'm interested in your software/web development services. Can we connect?", eyebrow: "Have a project in mind?", label: "Chat on WhatsApp" };
  var fm = { url: "https://sha4owx.in/contact", eyebrow: "Let\u2019s build something great.", label: "Start a Project" };
